...
def generate_grid_positions(num_motes, x_spacing, y_spacing):
    positions = []
    grid_size = int(num_motes**0.5)
    for i in range(grid_size):
        for j in range(grid_size):
            x = j * x_spacing
            y = i * y_spacing
            positions.append((x, y))
            if len(positions) >= num_motes:
                return positions
    return positions

def generate_simulation_config(num_motes, x_spacing, y_spacing, c_file, transmit_range, sim_name):
    positions = generate_grid_positions(num_motes, x_spacing, y_spacing)
...
for i in range(num_configs):
    ...
    x_spacing = random.randint(10, 50)  # Random spacing between motes in x direction
    y_spacing = random.randint(10, 50)  # Random spacing between motes in y direction
    ...
    config_content = generate_simulation_config(num_motes, x_spacing, y_spacing, c_file, transmit_range, sim_name)
