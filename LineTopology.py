...
def generate_line_positions(num_motes, x_spacing):
    positions = []
    y = 150  # Fixed y-coordinate for all motes
    for i in range(num_motes):
        x = i * x_spacing
        positions.append((x, y))
    return positions
def generate_simulation_config(num_motes, x_spacing, c_file, transmit_range, sim_name):
    positions = generate_line_positions(num_motes, x_spacing)
...
for i in range(num_configs):
    ...
    x_spacing = random.randint(10, 50)  # Random spacing between motes
    ...
    config_content = generate_simulation_config(num_motes, x_spacing, c_file, transmit_range, sim_name)
